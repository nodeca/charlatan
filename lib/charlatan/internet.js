...
/**
 * Charlatan.Internet.password() -> string
 *
 * Generate password
 *
 **/
exports.password = function (length, chars) {

  length = length || Helpers.rand(20, 5);
  chars = chars || 'abcdefghijklmnopqrstuwxyzABCDEFGHIJKLMNOPQRSTUWXYZ0123456789';

  var rnd = Math.pow(length, 4),
      value = new Array(length),
      len = chars.length;

  for (var i=0; i < length; i++) {
    value[i] = chars[Helpers.rand(rnd, length) % len];
  }
  return value.join('');    
};
