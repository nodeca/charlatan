...
var addLocale = exports.addLocale = function (name, data) {
  ...
  if (typeof data === 'string') {
    var file_name = '';
    var base_file_name = data.replace(/\.json|\.yml/, '');

    // use pre-compiled config, if exists
    if (Fs.existsSync(base_file_name + '.json')) {
      file_name = base_file_name + '.json';
    }
    else if (Fs.existsSync(base_file_name + '.yml')) {
      file_name = base_file_name + '.yml';
    }
    else {
      throw new Error('Config ' + data + ' not found');
    }
    data = require(file_name)[name];
  }
  locales[name] = data;
  return true;
};
