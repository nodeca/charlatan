...
var COMPILED_LOCALES_PATH = '../.locales/';
...
var addLocale = exports.addLocale = function (name, data) {
  ...
  if (data === undefined) {
    ...
    // try to use pre-compiled config, if exists
    var precompiled_file = Path.resolve(
      __dirname,
      COMPILED_LOCALES_PATH,
      name + '.json'
    );
    if (Fs.existsSync(precompiled_file)) {
      data = precompiled_file;
    }
    else {
...
exports.letterify = function(source) {
   return source.replace(/\?/g, function() {
     return Helpers.sample('ABCDEFGHIJKLMNOPQRSTUVWXTZ'.split(''));
   });
};
...
var locales_dir = Path.resolve(__dirname, '../config/locales') + '/';
Fs.readdirSync(locales_dir).forEach(function (file) {
  if (/\.yml/.test(file)) {
    avaliable_locales[file.replace('.yml', '')] = locales_dir + file;
  }
});
